...
namespace WebMVC.Bussiness
{
    public class MyRepository
    {
        ...
        private DataModelEntities entities = new DataModelEntities();
        ...
        public MyRepository()
        {
            this.entities = new DataModelEntities();
        }
        ...
        public List<Role> GetRoleForUser(string userName)
        {
            var member = GetUser(userName);
              List<Role> userinrole = new List<Role>();
            int userid = 0;
            if (member != null)
            {
                userid = GetUser(userName).UserId;
                userinrole = entities.UsersInRoles.Where(role => role.UserId == userid).Select(m => m.Role).ToList();
            }
            return userinrole;
        }
