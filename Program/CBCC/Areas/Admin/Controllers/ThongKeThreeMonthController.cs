...
namespace CBCC.Areas.Admin.Controllers
{
    public class ThongKeThreeMonthController : Controller
    {
        ...
        {
            var dt = CodeHelper.ConvertToDataTable(ThongKeService.ThongKeToanTP_BanBieu_ByMonthYear(month, year));
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                cells[row + i, 2].PutValue(dt.Rows[i]["Soluot"]);
                if (i == dt.Rows.Count - 1)
                {
                    cells[row + i + 1, 0].PutValue(i + 2);
                    cells[row + i + 1, 1].PutValue("Tổng cộng");
                    cells[row + i + 1, 2].PutValue(dt.Compute("sum(Soluot)", ""));
                }
            }
            ...
            // format title
            cells[1, 0].PutValue("Thống kê 3 tháng gần nhất");
            style = new Style();
